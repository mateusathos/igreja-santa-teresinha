...
function renderProgramacoes(data) {
  ...
  const visible = data.comunidades.filter((comunidade) => data.programacoes.some((item) => item.comunidade_id === comunidade.id));
  if (!visible.length) {
  ...
  list.innerHTML = visible.map((comunidade) => {
    const items = data.programacoes.filter((item) => item.comunidade_id === comunidade.id);
    return `<section id="comunidade-${comunidade.slug}" class="fade-in overflow-hidden rounded-lg border border-red-950/10 bg-white shadow-sm">
      <header class="border-b border-red-950/10 bg-red-950 px-5 py-4 text-white">
        ...
        <p class="mt-1 break-words text-xs leading-relaxed text-red-100">${escapeHtml(comunidade.endereco)}</p>
      </header>
